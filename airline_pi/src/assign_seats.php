...
class assignSeats{
    ...
    public function getPassengersByFlight($flightId){
        ...
        // Obtener todas las asignaciones de asientos ya hechas en assignSeats()
        $assignedSeats = $this->assignSeats($flightId);

        // Crear un mapa para relacionar passenger_id con su asiento asignado
        $seatMap = [];
        foreach ($assignedSeats as $assignment) {
            $pid = $assignment['passenger']['passenger_id'];
            $seat = $assignment['seat'];
            $seatMap[$pid] = $seat;
        }

        // Actualizar la info de pasajeros con los asientos asignados
        $formattedPassengers = [];
        foreach ($passengers as $p) {
            $pid = $p["passenger_id"];
            if (isset($seatMap[$pid])) {
                $seat = $seatMap[$pid];
                $p["seat_row"] = $seat["row"];
                $p["seat_column"] = $seat["column"];
                $p["seat_id"] = $seat["seatNumber"]; // simulamos seat_id con seatNumber
            }

            $formattedPassengers[] = [
                "passengerId" => (int)$p["passenger_id"],
                "dni" => $p["dni"],
                "name" => $p["name"],
                "age" => (int)$p["age"],
                "country" => $p["country"],
                "boardingPassId" => (int)$p["boarding_pass_id"],
                "purchaseId" => (int)$p["purchase_id"],
                "seatTypeId" => (int)$p["seat_type_id"],
                "seatId" => $p["seat_id"],
                "seatColumn" => $p["seat_column"],
                "seatRow" => $p["seat_row"]
            ];
        }
    ...
    public function enumeracionAsientos($flightId) {
        $pdo = $this->pdo;
        ...
        // decir que la variable indefinida(flight_id = ?) tendra el valor de $flightId
        $stmt->execute([$flightId]);
            ...
            foreach ($avionDisponibleEst as $clase) {
                ...
                if ($existeColumna) {
                    ...
                    foreach ($filas as $fila) {
                        ...
                        $numerosAsientos[$asiento] = $asientoActual++;
                    }
                }
            }
        }
